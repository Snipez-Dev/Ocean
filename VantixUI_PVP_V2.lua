...
local b64decode = (crypt and crypt.base64.decode) or (typeof(base64_decode)=='function' and base64_decode) or function(s) return s end
...
do
    ...
    function utility:Draw(class, properties)
        ...
        -- Create the underlying Roblox instance
        local inst
        if class == 'Text' then
            inst = newInstance('TextLabel')
            inst.BackgroundTransparency = 1
            inst.Size = newUDim2(0, 500, 0, 50)
        ...
        inst.Parent = library.guiRoot

        -- drawing table (forward-initialized so drawingObject closure can reference it)
        local drawing = {
            Object = nil;
            Children = {};
            ThemeColor = '';
        ...
        -- drawingObject: proxy that exposes the same API as a Drawing object
        -- but is backed by a Roblox Instance.
        local drawingObject = utility:DetectTableChange(
            function(_, i)
                if i == 'Color' then return dotColor
                elseif i == 'Transparency' then return dotTrans
                elseif i == 'OutlineColor' then return dotOutlineColor
                ...
                elseif i == 'Remove' then return function() pcall(function() inst:Destroy() end) end
                    ...
                    local ok, val = pcall(function() return inst[i] end)
            ...
            function(_, i, v)
                if i == 'Color' then
                    dotColor = v
                    if class == 'Text' then
                        pcall(function() inst.TextColor3 = v end)
                    else
                        pcall(function() inst.BackgroundColor3 = v end)
                    end
                ...
                elseif i == 'OutlineColor' then
                    dotOutlineColor = v
                    if class == 'Text' then
                        pcall(function() inst.TextStrokeColor3 = v end)
                    elseif stroke then
                        pcall(function() stroke.Color = v end)
                    end
                elseif i == 'Outline' then
                    if class == 'Text' then
                        pcall(function() inst.TextStrokeTransparency = v and 0.5 or 1 end)
                    end
                elseif i == 'Visible' then
                    pcall(function() inst.Visible = v end)
                elseif i == 'Size' then
                    if class == 'Text' then
                        pcall(function() inst.TextSize = v end)
                    else
                        if typeof(v) == 'Vector2' then
                            drawing.AbsoluteSize = v
                            pcall(function() inst.Size = newUDim2(0, v.X, 0, v.Y) end)
                        end
                    end
                elseif i == 'Position' then
                    if typeof(v) == 'Vector2' then
                        drawing.AbsolutePosition = v
                        pcall(function() inst.Position = newUDim2(0, v.X, 0, v.Y) end)
                    end
                elseif i == 'ZIndex' then
                    pcall(function() inst.ZIndex = v end)
                elseif i == 'Filled' then
                    if class == 'Square' then
                        if not v then
                            pcall(function() inst.BackgroundTransparency = 1 end)
                            if not stroke then
                ...
                elseif i == 'Thickness' then
                    if stroke then pcall(function() stroke.Thickness = v end) end
                ...
                elseif i == 'Font' then
                    if class == 'Text' then
                        pcall(function() inst.Font = Enum.Font.GothamMedium end)
                    end
                elseif i == 'Text' then
                    if class == 'Text' then
                        pcall(function() inst.Text = tostring(v) end)
                    end
                elseif i == 'Data' then
                    -- not supported in CoreGUI V2
                elseif i == 'PointA' or i == 'PointB' or i == 'PointC' then
                    -- Triangle cursor is not used in V2
                else
        ...
        library.drawings[drawingObject] = drawing
...
function library:init()
    ...
    function self.NewWindow(data)
        ...
        do
            ...
            do
                ...
                do
                    ...
                    g.Rotation = 0
                    g.Parent = objs.sat1.Object._inst
                end

                objs.sat2 = utility:Draw('Square', {
                    Size = newUDim2(1,0,1,0);
                    Color = c3new(0,0,0);
                    ZIndex = z+4;
                    Parent = objs.mainColor;
                ...
                do
                    ...
                    g.Rotation = 90
                    g.Parent = objs.sat2.Object._inst
                end
                ...
                do
                    ...
                    g.Color = ColorSequence.new({
                        ...
                        ColorSequenceKeypoint.new(1/6,  Color3.fromHSV(5/6,  1, 1)),
                        ColorSequenceKeypoint.new(2/6,  Color3.fromHSV(4/6,  1, 1)),
                        ColorSequenceKeypoint.new(3/6,  Color3.fromHSV(3/6,  1, 1)),
                        ColorSequenceKeypoint.new(4/6,  Color3.fromHSV(2/6,  1, 1)),
                        ColorSequenceKeypoint.new(5/6,  Color3.fromHSV(1/6,  1, 1)),
                        ColorSequenceKeypoint.new(1,    Color3.fromHSV(0,    1, 1)),
                    })
                    g.Rotation = 0
                    g.Parent = objs.hue.Object._inst
                end
                ...
                objs.trans = utility:Draw('Square', {
                    Size = newUDim2(1,0,1,0);
                    Color = c3new(0,0,0);
                    ZIndex = z+3;
                ...
                do
                    ...
                    g.Transparency = NumberSequence.new({NumberSequenceKeypoint.new(0,1), NumberSequenceKeypoint.new(1,0)})
                    g.Rotation = 90
                    g.Parent = objs.trans.Object._inst
                end
        ...
        function window:AddTab(text, order)
            ...
            do
                local autoSections = {}
                local function getAutoSection(side)
                    side = side or 1
                    if not autoSections[side] then
                        autoSections[side] = tab:AddSection('', side)
                    end
                    return autoSections[side]
                end
                function tab:AddToggle(d)    return getAutoSection(d and d.side):AddToggle(d) end
                function tab:AddSlider(d)    return getAutoSection(d and d.side):AddSlider(d) end
                function tab:AddButton(d)    return getAutoSection(d and d.side):AddButton(d) end
                function tab:AddList(d)      return getAutoSection(d and d.side):AddList(d) end
                function tab:AddColor(d)     return getAutoSection(d and d.side):AddColor(d) end
                function tab:AddBind(d)      return getAutoSection(d and d.side):AddBind(d) end
                function tab:AddBox(d)       return getAutoSection(d and d.side):AddBox(d) end
                function tab:AddText(d)      return getAutoSection(d and d.side):AddText(d) end
                function tab:AddSeparator(d) return getAutoSection(d and d.side):AddSeparator(d or {}) end
            end
...
function library:CreateSettingsTab(menu)
    ...
    mainSection:AddBind({text = 'Open / Close', flag = 'togglebind', nomouse = true, noindicator = true, bind = Enum.KeyCode.Insert, keycallback = function(key)
